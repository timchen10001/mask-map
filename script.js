...
class Data {
  ...
  _initData(r) {
    r.map((e) => {
      ...
      if (this.CountyMap[key] === undefined) {
        this.CountyMap[key] = new Map();
      } else if (this.CountyMap[key].has(info.town)) {
        this.CountyMap[key].get(info.town).push(e);
      } else {
        this.CountyMap[key].set(info.town, new Array());
      }
    });
  }
...
class App {
  ...
  _updateState() {
    ...
    const selectedCounty = countyArr.filter((option) => option.selected)[0]
      .value;
  ...
  _getHTML(coord, prop) {
    ...
    <li class="mask" data-id="${prop.id}">
      ...
      <p class="mask__adult">成人: ${prop.mask_adult}</p>
      <p class="mask__child">兒童: ${prop.mask_child}</p>
      <p class="note">${prop.note}</p>
      <p class="address">${prop.address}</p>
      <p class="phone">${prop.phone}</p>
      <p class="updated">${prop.updated}</p>
      <p class="coordinates--latitude hide">${coord[0]}</p>
      <p class="coordinates--longitude hide">${coord[1]}</p>
    </li>
    `;
  }
